...
@pytest.mark.asyncio()
async def test_async_chat_completion_with_raw_response_streaming_headers_accessible(
    ...
    ``AttributeError`` because the underlying ``AsyncStream`` object has no ``.headers``.
    """
    with vcr.use_cassette(
        "test_chat_completion_with_raw_response_streaming.yaml"
    ):
